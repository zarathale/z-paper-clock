...
- **LAYER-CONVENTIONS.md 066-cluster exemplar table drift** — the exemplar table still describes some matches as `panel-substring` where the actual match is now `marks-landing-self` or `marks-exact` after the 2026-05-10 marks-lookup ship. Worth a doc-refresh pass before the table misleads anyone authoring new pieces.
...
next_action: Send `CODE_PROMPT_preview-html-fold-groups.md` to Code (ready-for-code). Fold panel group-aware redesign for 066: pane-strip group (sum readout + Equal), tab-flap group (master sub-slider + 90° quick-set), cross-piece group (collapsed by default). In parallel: fix 068 fold authoring (missing pane→c1 fold line).
```
...
- 2026-05-10 (09:15): Bridge button shipped (`claude/preview-bridge-button`). New "→ Claude" button in the sidebar (between sidecar block and dropzone) POSTs the current piece's parsed state — `panelsFirst` (panels with polygon/bbox/centroid; folds; hingeTree with nodes serialised as plain object incl. fold ids; attachPoints; closureAttaches; marks), `cutouts` (when non-empty), `foldState` (per-fold slider values keyed by `dataset.pathId`), and the full `sidecar` JSON when loaded — to `http://localhost:7777/dump/preview`. Bridge server (`claude-work/scripts/preview_bridge.py`, written in Cowork) writes the body to `claude-work/state/preview-dump.json` for Claude to read directly. Page-load ping at `/ping` detects offline state and dims the button + updates title with the run command. `currentSidecar` promoted to module scope (alongside `currentAssembledFolds` / `currentAssembledTransform`); reset paths in `loadSvgFromString` (drag-drop / file-picker / cached-reload), `maybeLoadSidecar`, and `loadScene` updated to wipe it consistently. Verified end-to-end against piece 069 (panels=11, folds=10, hingeTree.nodes=11) and piece 071 (cutouts.length=2); offline / online / send-while-offline flash states all confirmed via headless preview. Replaces the copy-paste console-dump workflow. See `sessions/2026-05-10-0915_code_preview-bridge-button.md`.
- 2026-05-10 (08:20): Parser-marks-lookup shipped (`claude/parser-marks-lookup`) — `find_partner_feature` 8-step chain in `build_assembly_graph.py`; marks steps 4 (bare-letter exact / multi-instance collapse), 5a (`mark-<letter>` prefix), 5b (typed-landing self-reference), 8 (marks substring) inserted between attach-points and panel-substring fallback. Main-loop now globs `*.svg` per piece dir so split pieces (093a/093b) are reachable. Connection-graph regenerated: 30 valid authored edges (was 24); 093b→093a `marks-exact` resolves; 094→095/097→099 shift from panel-substring to mark-anchored. Two doc follow-ups parked: 097 period-suffix convention (see open questions) + LAYER-CONVENTIONS 066-cluster exemplar table drift. See `sessions/2026-05-10-0820_code_parser-marks-lookup.md`.
- 2026-05-10 (00:30): PR A shipped (`claude/preview-html-bench-cluster-foundation`) — cutouts as Shape holes, slider+number-entry, camera lock, TC click-drag, RGB axes + corner gizmo, worktable, Bench/Cluster scaffold. Manual visual check still needed (gizmo + TC handles + worktable need real browser, not headless). See `sessions/2026-05-10-0335_code_preview-html-bench-cluster-foundation.md`.
...
- 2026-05-10: marks-lookup ship updated `connection-graph.{json,md}` — 30 valid authored edges (was 24); 9 new pieces entered the graph (001/038/041/044/045/047/090/100/113 SVGs authored since last regen). Provenance of several edges shifted from panel-substring to mark-anchored (093b→093a, 094→095, 097→099, 065→066 h-tab, 068→069 h/i). The LAYER-CONVENTIONS.md 066-cluster exemplar table still describes old match types (e.g. `panel-substring` where the actual match is now `marks-landing-self`); needs a doc refresh pass. See `sessions/2026-05-10-0820_code_parser-marks-lookup.md`.
