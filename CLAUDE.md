...
- **Code sessions: do not start an HTTP server to verify `preview.html`.** Code runs in a worktree (`.claude/worktrees/<name>/`); starting a server there creates a path mismatch with any server Alan already has running at the repo root, and the port may not be bindable. Use `node --check preview.html` for syntax validation; use ripgrep for symbol checks; document the remaining browser-side checks in the Manual tests section for Alan to run post-merge. See "Orchestration Prompt Format" item 6 for the full two-tier verification protocol.
