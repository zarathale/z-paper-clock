...
| 0.6.9 | v1b: face graph (extend folds → split via polygon-clipping) + adjacency BFS + hinge tree + global / per-fold sliders | ready-for-code | 6.0 | — | Code | Multi-panel render with live fold animation | `CODE_PROMPT_preview-html-v1b.md` authored 2026-05-03 (clean start; the original draft is archived at `_archive/code-prompts/CODE_PROMPT_preview-html-v1b.md`). Eight tasks; `work/SPEC-REGIONS.md` drives the algorithmic framing; rotation pivot wraps the entire fold tree; verification scoped to `inbox/069.svg` (13 regions) + `inbox/066.svg` (22 regions). See `sessions/2026-05-03-0300_cowork_v1b-clean-start.md`. |
...
| 0.6.14 | Source-of-truth piece-id loader: load `work/pieces/NNN/NNN.svg` by piece id (text input with autocomplete or dropdown); reload button; optional `NNN.json` sidecar surfacing in side panel; drag-drop retained as fallback | ready-for-code | 1.5 | — | Code | Piece-selector UI, canonical loader path, reload button | `CODE_PROMPT_preview-html-source-of-truth.md`. Pairs with the 2026-05-03 filesystem restructure that made `work/pieces/NNN/NNN.svg` the single canonical export location for every piece. Independent of v1b — drop-in addition that doesn't touch the parser or render pipeline. |

**M0.6 verification.** Drop any per-piece SVG from `inbox/` or `work/pieces/` onto `preview.html` opened from `file://` and: silhouette renders cleanly via the Tier 1 layer; thickness slider responds and the slab measurably extrudes; axle rotation works on pieces with `axles` populated; banners surface authoring slips (missing layer, no cutaway inside silhouette, etc.); v1b folds (post-0.6.9) animate; cutouts subtract (post-0.6.10); architecture decision (post-0.6.13) is recorded in this doc and reflected in the SPEC.
...
*Last updated: 2026-05-03 (evening) — added M0.6.14 row for the source-of-truth piece-id loader, paired with the same-day filesystem restructure that locked `work/pieces/NNN/NNN.svg` as the single canonical export location per piece. M0.6 estimate aggregate updated implicitly. See `sessions/2026-05-03-2345_cowork_filesystem-restructure.md` and `CODE_PROMPT_preview-html-source-of-truth.md`.*
