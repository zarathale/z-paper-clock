...
| 0.6.9 | v1b: polygon cut + adjacency BFS + hinge hierarchy + global / per-fold sliders | not-started | 6.0 | — | Cowork (re-tighten prompt) + Code | Multi-panel render with live fold animation | `CODE_PROMPT_preview-html-v1b.md` is `status: draft` and depends on v1a's actual function signatures. Re-tighten before flipping to `ready-for-code`. |
