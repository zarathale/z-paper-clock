...
| 0.6.9 | v1b: polygon cut + adjacency BFS + hinge hierarchy + global / per-fold sliders | not-started | 6.0 | — | Cowork (clean-start prompt) + Code | Multi-panel render with live fold animation | The original draft prompt was archived 2026-05-03 to `_archive/code-prompts/CODE_PROMPT_preview-html-v1b.md` after the cumulative drift on `preview.html` since v1a (cut-layer convention, texture flip, back-face mirror, perf pass, thickness fix, axle rotation) outpaced its design assumptions. Author a clean-start prompt against the current `preview.html`, with `work/SPEC-REGIONS.md` as the upstream concept doc. See `sessions/2026-05-03-0200_cowork_v1b-archive.md`. |
