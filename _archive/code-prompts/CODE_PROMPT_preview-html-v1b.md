...
supersedes: _archive/code-prompts/CODE_PROMPT_preview-html-v1b-archived-draft.md (archived 2026-05-03; renamed during the 2026-05-06 root-folder reorg to disambiguate from this shipped prompt at the same canonical name)
---
...
4. `_archive/code-prompts/CODE_PROMPT_preview-html-v1b-archived-draft.md` — the archived earlier draft (renamed during the 2026-05-06 root-folder reorg to disambiguate from this shipped prompt). Useful for the polygon-cut algorithm sketch, edge-tag matching by midpoint distance, the sliver-filter calibration (Δ = 0.5% of silhouette-bbox diagonal), and the `069 → 13` / `066 → 22` expected counts. **Do not** reuse the variable names or task numbering — those targeted v1a-only state.
