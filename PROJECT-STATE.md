...
- **SVG layer authoring (panels-first).** The actual current bottleneck. 17 panels-first SVGs authored across the anchor / pendulum / bob clusters (065-072 + 094-100 + 110); next pulls in §II.B gear train (087 escape-wheel underway). Conventions stable post-DECISIONS #7; LAYER-CONVENTIONS.md is the live reference. Per-piece sidecars now carry `function` (mechanism geometry), `connections.inferred[]` (learned cross-piece edges, DECISIONS #10), and `assembled.folds` (per-fold assembled angles, DECISIONS #11) blocks.
