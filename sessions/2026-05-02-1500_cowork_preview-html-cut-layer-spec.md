...
3. Code session executes `CODE_PROMPT_preview-html-cut-layer.md` against **every `.svg` currently in `inbox/`** (the project's transient working zone — these test pieces haven't been promoted to `work/pieces/` yet because preview-html is pre-pipeline tooling Alan loads via drag-drop). Alan has been steadily authoring + dropping in new SVGs that follow the new naming convention, so the test set is intentionally open-ended: Code should `ls inbox/*.svg` at session start and walk through each one, reporting which tier (layer / alpha / path-fallback) each piece resolves through and any banners/warnings it produces. Specific pieces with known expected behavior, as anchors: `inbox/069.svg` is the canonical Tier-1 case (re-authored 2026-05-02 with `<g id="silhouette">` containing `<rect id="mask">` + `<path id="cutaway">`); `inbox/066.svg` and `inbox/070.svg` are legacy / no-silhouette-layer Tier-2/3 cases; `inbox/067.svg` exercises Tier 1 if Alan re-authored it before the session, otherwise Tier 2. Pieces beyond those (065, 071, 072, and anything else Alan adds before the session) should be tested too — they're the real shakedown for whether the convention holds across his actual authoring workflow.
