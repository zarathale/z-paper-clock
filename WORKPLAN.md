...
status: active
last_updated: 2026-05-04
next_action: Hand CODE_PROMPT_preview-html-face-graph-diagnostics.md to a Code session — purely additive instrumentation (shared-edge index, face-graph JSON dump, 2D debug overlay over the 3D view) so we can inspect 066's actual face graph before committing to shared-edge / cut-trim / multi-tag as the algorithm direction. Algorithm refactor (with cleanups: passive → kind:driven|structural, tightened picker) follows in a second prompt informed by what the diagnostics surface.
```
...
- 2026-05-04 (late morning): **diagnose-first pivot for the post-marker-bound work.** Sat down to write a shared-edge topology refactor (the design pivot from the 0815 session) and surfaced two reservations: pickClosestSharedEdge's one-criterion picker is silently wrong on coincidental near-line interior seams, and `passive` semantics get narrowed enough to deserve a rename. Drilling into the option space surfaced more alternatives than expected — shared-edge alone, shared-edge + cut-trim together, multi-tag during cuts, panels-layer authoring, **diagnose-first**. Picked diagnose-first: instrument `buildFaceGraph` to expose a face-graph JSON dump + 2D SVG diagnostic overlay (regions in unique colors, shared edges, fold authored segments, marker centroids, top-1 candidate per fold flagged green/amber). Algorithm refactor deferred to a second prompt informed by what the diagnostic harness actually shows on 066. Cleanups (passive → `kind: driven | structural`, direction/segment/length filters in the picker) ride along with the algorithm prompt, not the diagnostic. Stray `work/pieces/066/001.svg` (byte-identical duplicate of `001/001.svg`) cleaned up. `CODE_PROMPT_preview-html-face-graph-diagnostics.md` drafted at repo root (status: ready-for-code). See `sessions/2026-05-04-1100_cowork_face-graph-diagnostics.md`.
...
*Last updated: 2026-05-04 (late morning) — **diagnose-first pivot.** Sat down to write the shared-edge topology refactor and surfaced enough option-space (shared-edge alone, +cut-trim, multi-tag, panels, diagnose-first) plus enough genuine reservations on the picker's one-criterion scoring that committing the algorithm refactor felt premature. Pivoted to a diagnostic-harness CODE_PROMPT that builds the shared-edge index for inspection only, ships a face-graph JSON dump + 2D SVG overlay over the 3D view, and lets us look at 066's actual face graph before picking the algorithm. `CODE_PROMPT_preview-html-face-graph-diagnostics.md` ready-for-code at repo root. Stray `work/pieces/066/001.svg` cleaned up (byte-identical duplicate of `001/001.svg`). See `sessions/2026-05-04-1100_cowork_face-graph-diagnostics.md`.*
